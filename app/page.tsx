...
type GithubCommitFile = { path: string; status: string; additions: number; deletions: number };
type GithubCommit = { id: string; short_sha: string; message: string; author_name: string; committed_at: string | null; additions: number; deletions: number; file_count: number; files_truncated: boolean; files: GithubCommitFile[] };
...
type CardDetail = { checklists: Checklist[]; comments: Comment[]; attachments: Attachment[]; activity: Activity[]; cover_attachment_id: string | null; cover_mode: 'full' | 'top'; background_image_url: string | null; unread_mention_source_ids: string[]; watching: boolean; github_commits: GithubCommit[] };
...
export default function Home() {
  ...
  const [githubCommitDraft, setGithubCommitDraft] = useState('');
  const [isAttachingGithubCommit, setAttachingGithubCommit] = useState(false);
  ...
  useEffect(() => {
    ...
    void fetch(`${API_URL}/v1/cards/${selectedCardId}/details`)
      ...
      .then((detail) => {
        ...
        setGithubCommits(detail.github_commits ?? []);
  ...
  async function attachGithubCommit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selected || !githubCommitDraft.trim() || isAttachingGithubCommit) return;
    if (persistence !== 'connected' || typeof selected.id !== 'string') { showToast('Для коммитов нужно подключение к серверу'); return; }
    setAttachingGithubCommit(true);
    try {
      const response = await fetch(`${API_URL}/v1/cards/${selected.id}/github-commits`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sha: githubCommitDraft.trim() }) });
      const payload = await response.json().catch(() => null) as GithubCommit | { message?: string } | null;
      if (!response.ok) throw new Error(payload && 'message' in payload && payload.message ? payload.message : 'Не удалось прикрепить коммит');
      setGithubCommits((current) => [payload as GithubCommit, ...current]);
      setGithubCommitDraft('');
      showToast('Коммит прикреплён к карточке');
    } catch (error) { showToast(error instanceof Error ? error.message : 'Не удалось прикрепить коммит'); }
    finally { setAttachingGithubCommit(false); }
  }
  function removeGithubCommit(commit: GithubCommit) {
    if (!selected || persistence !== 'connected' || typeof selected.id !== 'string') return;
    setGithubCommits((current) => current.filter((item) => item.id !== commit.id));
    void fetch(`${API_URL}/v1/cards/${selected.id}/github-commits/${commit.id}`, { method: 'DELETE' })
      .then((response) => { if (!response.ok) throw new Error('commit delete failed'); showToast('Коммит откреплён'); })
      .catch(() => { setCardDetailRevision((current) => current + 1); showToast('Не удалось открепить коммит'); });
  }
      ...
      <section className="task-modal" role="dialog" aria-modal="true" aria-label="Детали задачи" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="task-layout">
          <div className="task-content">
            ...
            {account && <section className="github-commits"><div className="section-heading"><h3>Коммиты</h3><span>{githubCommits.length || '—'}</span></div>{githubCommits.length ? <div className="github-commit-list">{githubCommits.map((commit) => <article key={commit.id}><header><code>{commit.short_sha}</code><span>{commit.author_name}{commit.committed_at && <> · {new Date(commit.committed_at).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</>}</span>{!isPublicViewer && <button type="button" onClick={() => removeGithubCommit(commit)} aria-label={`Открепить коммит ${commit.short_sha}`}>×</button>}</header><p>{commit.message}</p><small><b className="github-additions">+{commit.additions}</b><b className="github-deletions">−{commit.deletions}</b><span>{commit.file_count} файлов</span></small>{commit.files.length > 0 && <ul>{commit.files.map((file) => <li key={`${commit.id}-${file.path}`}><code>{file.path}</code><span><b className="github-additions">+{file.additions}</b><b className="github-deletions">−{file.deletions}</b></span></li>)}</ul>}{commit.files_truncated && <em>Показаны первые {commit.files.length} файлов.</em>}</article>)}</div> : <p className="empty-attachments">Прикреплённых коммитов пока нет.</p>}{!isPublicViewer && <form className="github-commit-form" onSubmit={attachGithubCommit}><input value={githubCommitDraft} onChange={(event) => setGithubCommitDraft(event.target.value)} placeholder="SHA уже запушенного коммита" aria-label="SHA GitHub-коммита" minLength={7} maxLength={64} pattern="[0-9a-fA-F]{7,64}" /><button type="submit" disabled={isAttachingGithubCommit || githubCommitDraft.trim().length < 7}>{isAttachingGithubCommit ? 'Читаем…' : '＋ Коммит'}</button></form>}</section>}
