        ...
        <div className="task-layout">
          <div className="task-content">
            ...
            <div className="card-property-area">
              <div className="card-quick-actions">{renderCardMilestoneControl()}{authState === 'signed-in' && <button type="button" className={`quick-action card-watch-toggle ${isWatchingCard ? 'active' : ''}`} onClick={toggleCardWatch} title={isWatchingCard ? 'Отключить уведомления об этой карточке' : 'Подписаться на изменения карточки'} aria-label={isWatchingCard ? 'Отключить уведомления об этой карточке' : 'Подписаться на изменения карточки'}>{isWatchingCard ? '◉' : '◌'}</button>}<button className="quick-action" onClick={openDiagram} title="Схема" aria-label="Открыть схему">⌁</button><button className={`quick-action ${sidebarPanel === 'labels' ? 'active' : ''}`} onClick={() => { setExistingLabelsOnly(false); setSidebarPanel((current) => current === 'labels' ? null : 'labels'); }} title="Метки" aria-label="Настроить метки"><BoardToolbarIcon type="labels" /></button><button className={`quick-action ${sidebarPanel === 'due' ? 'active' : ''}`} onClick={() => setSidebarPanel((current) => current === 'due' ? null : 'due')} title={selected.dueAt ? `Дедлайн: ${formatDue(selected.dueAt)}` : 'Дедлайн'} aria-label="Настроить дедлайн">◷</button><button className={`quick-action ${sidebarPanel === 'background' ? 'active' : ''}`} onClick={() => setSidebarPanel((current) => current === 'background' ? null : 'background')} title="Фон карточки" aria-label="Настроить фон карточки">▧</button><button className={`quick-action ${sidebarPanel === 'public-visibility' ? 'active' : ''}`} onClick={() => setSidebarPanel((current) => current === 'public-visibility' ? null : 'public-visibility')} title="Доступ" aria-label="Настроить доступ к карточке">◉</button></div>
