...
import { ChangeEvent, ClipboardEvent as ReactClipboardEvent, DragEvent as ReactDragEvent, FormEvent, MouseEvent as ReactMouseEvent, MouseEventHandler, PointerEvent as ReactPointerEvent, ReactNode, useEffect, useMemo, useRef, useState } from 'react';
import './auth.css';
    ...
    return <div className="board-labels-control">
      ...
      {isBoardLabelsOpen && <div className="board-labels-popover">
        ...
        <div className="board-labels-list">{boardLabels.length ? boardLabels.map((label) => <article key={label.id}>
          {editingBoardLabel?.id === label.id ? <form className="label-editor" onSubmit={saveBoardLabel}>
        ...
        {!isPublicViewer && <form className="label-editor label-create-editor" onSubmit={createLabel}>
          ...
          <div className="label-editor-actions"><button type="button" className="shake-colors-button" onClick={() => { setNewLabelColor(randomChipColor()); setNewLabelIconColor(randomChipColor()); }}>Взболтнуть</button><button type="submit" disabled={!newLabelName.trim() || isSavingLabel}>{isSavingLabel ? 'Создаём…' : 'Создать метку'}</button></div>
        </form>}
      </div>}
    </div>;
  }
