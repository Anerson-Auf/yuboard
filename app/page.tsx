...
export default function Home() {
  ...
  return <main className={`app-shell dark ${view === 'home' ? 'home-mode' : ''} ${isPublicViewer ? 'public-viewer' : ''} ${boardBackgroundUrl && view === 'board' && !boardBackgroundFailed ? 'has-board-background' : ''} ${usesDefaultBoardBackground ? 'default-board-background' : ''}`} style={boardBackgroundStyle}>
    ...
    {view === 'home' ? <section className="home-screen workspace-cards-screen">
      ...
      {account && boardId && <><PinnedCardsShelf storageKey={`flowboard.pinned-cards.${account.user.id}.${boardId}`} cards={columns.flatMap((column) => column.cards.map((card) => ({ id: String(card.id), title: card.title, listTitle: column.title, completed: Boolean(card.completedAt) })))} onOpen={(cardId) => { const card = columns.flatMap((column) => column.cards).find((item) => String(item.id) === cardId); if (card) openCard(card); }} />{parkingStorageKey && <ParkingShelf storageKey={parkingStorageKey} columns={columns.map((column) => ({ id: String(column.id), title: column.title }))} cards={parkingCards} onChange={setParkingCards} onMoveToBoard={restoreParkingCard} />}</>}
