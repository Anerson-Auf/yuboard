  ...
  function addColumn(position?: FreeformPosition) {
    ...
    if (persistence === 'connected' && boardId) {
      void fetch(`${API_URL}/v1/boards/${boardId}/lists`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title }) })
        .then(async (response) => { if (!response.ok) throw new Error('save failed'); return response.json() as Promise<{ id: string; title: string; grid_column: number; grid_row: number; card_limit: number }>; })
        .then((saved) => {
          const nextColumn: Column = { id: saved.id, title: saved.title, cardLimit: saved.card_limit ?? 0, cards: [] };
    ...
    setColumns((current) => [...current, { id: current.length + 1, title, cardLimit: 0, cards: [] }]); showToast('Колонка добавлена');
  }
