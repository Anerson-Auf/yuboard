...
type Card = { id: EntityId; title: string; description?: string; dueAt?: string; coverAttachmentId?: string; coverUrl?: string; coverMode?: 'full' | 'top'; backgroundImageUrl?: string; completedAt?: string; isPublic?: boolean; labels: Label[]; checklist?: string; comments?: number; attachments?: number; members: Member[] };
...
function MarkdownDescription({ value }: { value: string }) {
  ...
  while (index < lines.length) {
    ...
    if (heading) { const Tag = `h${heading[1].length}` as keyof JSX.IntrinsicElements; blocks.push(<Tag key={`heading-${index}`}>{inlineMarkdown(heading[2])}</Tag>); index += 1; continue; }
    if (/^\s*>\s?/.test(line)) {
      const quote: string[] = [];
    ...
    if (unordered || ordered) {
      ...
      const List = orderedList ? 'ol' : 'ul'; blocks.push(<List key={`list-${index}`}>{items.map((item, itemIndex) => <li key={itemIndex}>{inlineMarkdown(item)}</li>)}</List>);
      continue;
    }
    blocks.push(<p key={`paragraph-${index}`}>{inlineMarkdown(line)}</p>); index += 1;
  }
...
function activityDetail(detail: string) {
  // Earlier records prefixed checklist item titles with the legacy checklist
  // name "Недоделано". It is not useful activity information.
  return detail.replace(/^(?:не\s?доделано)\s*:\s*/iu, '');
}
...
export default function Home() {
  ...
  })), [columns, currentMember.id, filterMode, query]);
  ...
  function applyBoard(data: ApiBoard) {
    setColumns(data.lists.map((list) => ({ id: list.id, title: list.title, cards: list.cards.map((card) => ({ id: card.id, title: card.title, description: card.description, isPublic: card.is_public, backgroundImageUrl: card.background_image_url ?? undefined, dueAt: card.due_at ?? undefined, coverAttachmentId: card.cover_attachment_id ?? undefined, coverUrl: card.cover_url ?? undefined, coverMode: card.cover_mode, completedAt: card.completed_at ?? undefined, checklist: card.checklist_total ? `${card.checklist_completed}/${card.checklist_total}` : undefined, comments: card.comment_count || undefined, attachments: card.attachment_count || undefined, labels: card.labels, members: card.assignees.map(memberFromApi) })) })));
  ...
  function saveBoardBackground(event: FormEvent) {
    ...
    void fetch(`${API_URL}/v1/boards/${boardId}/background`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ background_image_url }) })
            ...
            <section className="conversation-panel" aria-label="Комментарии и активность">
              ...
              {isDetailsLoading ? <p className="detail-loading">Загружаем сообщения…</p> : <div className="conversation-scroll">
                {comments.filter((comment) => !comment.parent_comment_id).map((comment) => <div className="comment-thread" key={comment.id}>
                  <div className="comment"><Avatar member={comment.author_id === account?.user.id ? currentMember : { id: `comment-${comment.id}`, initials: comment.author_name.slice(0, 2).toUpperCase() || 'У', color: 'mint', name: comment.author_name, avatarUrl: comment.author_avatar_url }} /><div className="comment-body">{editingCommentId === comment.id ? <form className="comment-edit" onSubmit={(event) => { event.preventDefault(); saveCommentEdit(comment); }}><textarea autoFocus value={commentEditDraft} onChange={(event) => setCommentEditDraft(event.target.value)} maxLength={10000} /><div><button type="submit">Сохранить</button><button type="button" onClick={() => setEditingCommentId(null)}>Отмена</button></div></form> : <><header><b>@{comment.author_name}</b><time>{comment.created_at ? new Date(comment.created_at).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'только что'}{comment.edited_at && ' · изменено'}</time></header><div className="comment-text"><MarkdownDescription value={comment.body} /></div><div className="comment-actions"><button onClick={() => { setReplyToCommentId(comment.id); setCommentDraft(''); }}>Ответить</button>{comment.author_id === account?.user.id && <><button onClick={() => beginCommentEdit(comment)}>Изменить</button><button onClick={() => removeComment(comment)}>Удалить</button></>}</div></>}</div></div>
                  {comments.filter((reply) => reply.parent_comment_id === String(comment.id)).map((reply) => <div className="comment comment-reply" key={reply.id}><Avatar member={reply.author_id === account?.user.id ? currentMember : { id: `comment-${reply.id}`, initials: reply.author_name.slice(0, 2).toUpperCase() || 'У', color: 'mint', name: reply.author_name, avatarUrl: reply.author_avatar_url }} /><div className="comment-body">{editingCommentId === reply.id ? <form className="comment-edit" onSubmit={(event) => { event.preventDefault(); saveCommentEdit(reply); }}><textarea autoFocus value={commentEditDraft} onChange={(event) => setCommentEditDraft(event.target.value)} maxLength={10000} /><div><button type="submit">Сохранить</button><button type="button" onClick={() => setEditingCommentId(null)}>Отмена</button></div></form> : <><header><b>@{reply.author_name}</b><time>{reply.created_at ? new Date(reply.created_at).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'только что'}{reply.edited_at && ' · изменено'}</time></header><div className="comment-text"><MarkdownDescription value={reply.body} /></div><div className="comment-actions">{reply.author_id === account?.user.id && <><button onClick={() => beginCommentEdit(reply)}>Изменить</button><button onClick={() => removeComment(reply)}>Удалить</button></>}</div></>}</div></div>)}</div>)}
                {!comments.length && <p className="empty-comments">Пока нет сообщений. Начните обсуждение.</p>}
                {activity.map((item) => <div className="activity-message" key={item.id}><i>Console</i><p><b>@{item.actor_name ?? 'Deleted user'}</b> {activityLabel(item.action)}{activityDetail(item.detail) && <> · {activityDetail(item.detail)}</>}<small>{new Date(item.created_at).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</small></p></div>)}
              </div>}
              <form className="comment-composer" onSubmit={addComment}>{replyToCommentId && <div className="replying-to">Ответ на сообщение <button type="button" onClick={() => setReplyToCommentId(null)}>×</button></div>}<textarea className={`media-drop-target ${isUploadingAttachment ? 'uploading' : ''}`} value={commentDraft} onChange={(event) => setCommentDraft(event.target.value)} onDragOver={(event) => { if (event.dataTransfer.types.includes('Files')) event.preventDefault(); }} onDrop={(event) => handleMediaDrop(event, 'comment')} onPaste={(event) => handleMediaPaste(event, 'comment')} maxLength={10000} placeholder={replyToCommentId ? 'Написать ответ…' : 'Написать комментарий или перетащить медиа…'} aria-label="Написать комментарий" /><button className="add-card" type="submit" disabled={isSendingComment || !commentDraft.trim()}>{isSendingComment ? 'Отправка…' : replyToCommentId ? 'Ответить' : 'Отправить'}</button></form>
            </section>
          </aside>
        </div>
      </section>
