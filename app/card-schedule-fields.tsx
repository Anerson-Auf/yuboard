...
export default function CardScheduleFields({ cardId, startAt, dueAt, canEdit, onChange }: { cardId: string; startAt?: string; dueAt?: string; canEdit: boolean; onChange: (patch: { startAt?: string; dueAt?: string }) => void }) {
  ...
  const update = (field: 'start' | 'due', value: string) => {
    ...
    const request = field === 'start'
      ? fetch(`/v1/cards/${cardId}`, { method: 'PATCH', headers: jsonHeaders(), body: JSON.stringify({ start_at: value ? `${value}T09:00:00` : null }) })
      : fetch(`/v1/cards/${cardId}/due-date`, value ? { method: 'PATCH', headers: jsonHeaders(), body: JSON.stringify({ due_at: `${value}T18:00:00` }) } : { method: 'DELETE', headers: jsonHeaders() });
    void request.then((response) => { if (!response.ok) throw new Error('date save failed'); if (field === 'start') { setStart(value); onChange({ startAt: value ? `${value}T09:00:00` : undefined }); } else { setDue(value); onChange({ dueAt: value ? `${value}T18:00:00` : undefined }); } }).catch(() => { setStart(dateValue(startAt)); setDue(dateValue(dueAt)); }).finally(() => setSaving(false));
  };
