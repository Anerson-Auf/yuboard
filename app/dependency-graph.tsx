...
export default function DependencyGraph({ boardId, nodes, canEdit, onOpenCard }: { boardId: string; nodes: DependencyGraphNode[]; canEdit: boolean; onOpenCard: (cardId: string) => void }) {
  ...
  return <section className="dependency-graph" aria-label="Граф зависимостей карточек" onPointerDownCapture={(event) => {
    ...
    <div ref={viewportRef} className={`dependency-graph-scroll ${draft ? 'is-linking' : ''}`} onPointerDown={(event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      {canvasMenu && <div className="dependency-menu dependency-canvas-menu" style={menuPosition(canvasMenu.client, 62)}><button type="button" onClick={() => { setSourceMenu(canvasMenu); setCanvasMenu(null); }}>↗ Создать зависимость</button></div>}
      {sourceMenu && <div className="dependency-menu dependency-source-picker" style={menuPosition(sourceMenu.client, 432)}><b>От какой карточки?</b>{nodes.map((node) => <button key={node.id} type="button" onClick={() => begin(node.id, sourceMenu.canvas)}><small>{node.listTitle}</small>{node.title}</button>)}</div>}
