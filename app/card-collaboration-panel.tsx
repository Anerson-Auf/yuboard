...
export default function CardCollaborationPanel({ cardId, canEdit, candidates, onOpenCard, onDescriptionRestore }: { cardId: string; canEdit: boolean; candidates: Candidate[]; onOpenCard: (cardId: string) => void; onDescriptionRestore: (description: string) => void }) {
  ...
  const load = () => {
    void Promise.all([
      fetch(`/v1/cards/${cardId}/relations`).then((response) => response.ok ? response.json() as Promise<Relation[]> : []),
      fetch(`/v1/cards/${cardId}/description-versions`).then((response) => response.ok ? response.json() as Promise<Version[]> : []),
  ...
  const createRelation = () => {
    ...
    setSaving(true);
    void fetch(`/v1/cards/${cardId}/relations`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ target_card_id: targetId, relation_type: relationType }) })
      .then(async (response) => { if (!response.ok) throw new Error('relation create failed'); return response.json() as Promise<Relation>; })
      .then(() => { setTargetId(''); load(); })
      .finally(() => setSaving(false));
  };

  const removeRelation = (relationId: string) => {
    void fetch(`/v1/cards/${cardId}/relations/${relationId}`, { method: 'DELETE' }).then((response) => { if (response.ok) setRelations((current) => current.filter((relation) => relation.id !== relationId)); });
  };

  const restoreVersion = (version: Version) => {
    if (!canEdit || isSaving) return;
    setSaving(true);
    void fetch(`/v1/cards/${cardId}/description-versions/${version.id}/restore`, { method: 'POST' })
      .then(async (response) => { if (!response.ok) throw new Error('description restore failed'); return response.json() as Promise<{ description: string }>; })
