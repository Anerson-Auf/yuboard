...
export default function CardPollsPanel({ cardId, canEdit }: { cardId: string; canEdit: boolean }) {
  ...
  return <section className="card-polls-panel"><header><h3>Голосования</h3><span>{polls.length}</span></header>{polls.map((poll) => { const total = poll.options.reduce((sum, option) => sum + option.votes, 0); return <article key={poll.id}><b>{poll.question}</b><small>@{poll.created_by} · {new Date(poll.created_at).toLocaleDateString('ru-RU')}</small><div>{poll.options.map((option) => <button type="button" className={option.voted ? 'voted' : ''} onClick={() => vote(poll, option)} key={option.id}><span>{option.title}</span><i style={{ width: `${total ? Math.round(option.votes / total * 100) : 0}%` }} /><em>{option.votes}</em></button>)}</div></article>; })}{canEdit && <form onSubmit={(event) => { event.preventDefault(); create(); }}><input value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={300} placeholder="Вопрос для голосования" /><textarea value={options} onChange={(event) => setOptions(event.target.value)} placeholder={'Вариант 1\nВариант 2'} /><button disabled={isCreating || !question.trim()}>Создать голосование</button></form>}{!polls.length && !canEdit && <p>Голосований пока нет.</p>}</section>;
}
