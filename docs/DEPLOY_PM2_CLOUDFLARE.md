...
Ни один чужой PM2-процесс не перезапускается. Скрипт использует отдельный `PM2_HOME=$HOME/.pm2-flowboard`, поэтому Flowboard не смешивается даже со списком существующих PM2-приложений. Миграции базы запускаются API автоматически при старте.
