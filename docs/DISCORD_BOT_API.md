...
This API is for a bot that imports Discord suggestions into one selected Flowboard list. It is intentionally not a general account API: a token can create cards only in its configured list and can add Discord comments only to cards on that board. It cannot read private boards, manage people, change permissions, or delete content.

## Create a token

Open the project, click **Discord API**, choose the list for suggestions, and create a token. Copy it immediately; Flowboard stores only its SHA-256 digest and never shows the original token again. Revoking the token takes effect immediately.
