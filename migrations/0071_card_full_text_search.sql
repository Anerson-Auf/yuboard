...
CREATE INDEX IF NOT EXISTS cards_search_document_idx
    ON cards USING gin (to_tsvector('simple', coalesce(title, '') || ' ' || coalesce(description, '')));
...
CREATE INDEX IF NOT EXISTS checklist_items_search_document_idx
    ON checklist_items USING gin (to_tsvector('simple', coalesce(title, '') || ' ' || coalesce(description, '')));
